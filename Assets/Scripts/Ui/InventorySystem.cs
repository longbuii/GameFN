...
public class InventorySystem : MonoBehaviour
{
    ...
    public void consume(int _id)
    {
        if (_id >= 0 && _id < items.Count)
        {
            ...
            {
                Item item = items[_id].obj.GetComponent<Item>();
                ItemContents itemContents = items[_id].obj.GetComponent<ItemContents>();

                // Setting to the Type of Item 
                switch (item.consumableType)
                {
                    case Item.ConsumableType.Health:
                        if (itemContents != null)
                        {
                            itemContents.UseHealthItem();
                            Debug.Log("Used item with consumableType Health and get: " + itemContents.healthValue);
                        }
                        break;

                    case Item.ConsumableType.Experience:
                        if (itemContents != null)
                        {
                            itemContents.UseExperienceItem();
                            Debug.Log("Used item with consumableType Experience and get: " + itemContents.experienceValue);
                        }
                        break;

                    case Item.ConsumableType.Defense:
                        if (itemContents != null)
                        {
                            itemContents.UseDefenseItem();
                            Debug.Log("Used item with consumableType Defense and get: " + itemContents.defenseValue);
                        }
                        break;

                    default:
                        Debug.LogWarning("Unknown consumable type: " + item.consumableType);
                        break;
                }
